/**
 * @fileoverview contains all logic for soccerprespin.
 */
...
@Component({
    selector: 'app-pre-soccer-spin',
    templateUrl: './pre-soccer-spin.component.html',
    styleUrls: ['./pre-soccer-spin.component.css'],
    providers: [PreSoccerSpinService, SharedMethodsService]
})
export class PreSoccerSpinComponent implements OnInit, AfterViewInit, OnDestroy {
    addMatch: boolean;
     ...
     *
     * @constructor
     * @param {soccerPreSpinData} spinData - instance of PreSoccerSpinService service.
     ...
     * @param {SharedmethodsService} Sharedmethods - instance of SharedmethodsService to get
     * common functions in both pre spin and live spin.
     * @param {analytics} AnalyticsService - instance of AnalyticsService service.
     */
    constructor(
        private soccerPreSpinData: PreSoccerSpinService,
    ...
    */
    ...
    /**
    *
    * This function subscribes and gets data from getData method in soccerprespinService.
    */
    getDataFromService() {
        this.soccerPreSpinData.getData()
            .subscribe(resData => {
    ...
    */
    spin() {
        this.analytics.sendAnalytics(environment.widgetsCategories.soccerPreSpin, 'PreCombiSpin.Spin', '');
        const self = this;
        this.isSpin = true;
    ...
    lock(ind) {
        if (!this.isSpin) {
            ...
            if (this.marketsDisplay[ind]['isLock']) {
                this.analytics.sendAnalytics(environment.widgetsCategories.soccerPreSpin, 'PreCombiSpin.LockClick', '');
            } else {
                this.analytics.sendAnalytics(environment.widgetsCategories.soccerPreSpin, 'PreCombiSpin.UnlockClick', '');
            }
        }
    }
    /**
    ...
    */
    oddsclick() {
        this.analytics.sendAnalytics(environment.widgetsCategories.soccerPreSpin, 'PreCombiSpin.OddsBar', '');
    }
    /**
    *
    ...
    */
    add() {
        this.analytics.sendAnalytics(environment.widgetsCategories.soccerPreSpin, 'PreCombiSpin.AddMatch', '');
        this.returns = 10;
        this.marketsFilter = [];
        this.showErrorMessage = false;
        this.marketsFilter = [];
    ...
    */
    isRemove(ind) {
        this.analytics.sendAnalytics(environment.widgetsCategories.soccerPreSpin, 'PreCombiSpin.Xclick', '');
        this.returns = 10;
    ...
    */
    AddtoBetslip() {
        this.sharedMethodsService.addAllToBetslip(this.marketsDisplay, this.kambiService, (selectedids) => {
            this.analytics.sendAnalytics(environment.widgetsCategories.soccerPreSpin, 'PreCombiSpin.AddAllToBetSlip', selectedids);
        });
    }
    /**
    *
